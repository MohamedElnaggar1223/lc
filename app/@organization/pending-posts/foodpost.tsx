import { Food, PostType, usePostsStore } from "@/lib/store"
...
export default function FoodPost({ post, postDetails, index, pending }: Props) 
{
        ...
        <div className='flex px-8 w-full gap-4 items-center justify-between'>
            ...
            {isEditing ? (
                <div className='flex gap-4'>
                    <input type='number' value={parseInt(details.quantity)} onChange={e => setDetails(prev => ({ ...prev, quantity: details.type === 'Vegetables' || details.type === 'Fruits' ? e.target.value + 'KG' : e.target.value.replace("KG", "") }))} />
                    <select value={details.type} onChange={e => setDetails(prev => ({ ...prev, type: (e.target.value as "Vegetables" | "Fruits" | "Baked Goods" | "Fresh Meals" | "Canned Food") }))}>
                ...
                <div className='flex px-12 flex-1 flex-col gap-4'>
                    ...
                    <p className='text-[rgba(0,59,51,1)] text-2xl'>Name: {details.name}</p>
                </div>
            )}
