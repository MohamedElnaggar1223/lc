import { MedicalSupplies, PostType, usePostsStore } from "@/lib/store"
...
type Props = {
    post: PostType
    postDetails: MedicalSupplies
    index: number
    create?: boolean
    pending?: boolean
}

export default function MedicalSuppliesPost({ post, postDetails, index, pending }: Props) 
{
    ...
    return (
        <section className='flex px-8 w-full gap-4 items-center justify-between'>
            ...
            {isEditing ? details.type === 'Medication' ? (
                <div className='flex gap-4'>
                <div className='flex flex-col gap-4'>
                    <input type='number' value={details.quantity} onChange={e => setDetails(prev => ({ ...prev, quantity: e.target.value }))} />
                    <select value={details.gender} onChange={e => setDetails(prev => ({ ...prev, gender: (e.target.value as 'Male' | 'Female') }))}>
                        <option value='Male'>Male</option>
                        <option value='Female'>Female</option>
                    </select>
                    <select value={details.type} onChange={e => setDetails(prev => ({ ...prev, type: (e.target.value as "Medication" | "Medical Devices" | "Medical Equipment") }))}>
                ...
                <div className='flex flex-col gap-4'>
                    <select value={details.medicationType} onChange={e => setDetails(prev => ({ ...prev, medicationType: (e.target.value as "Painkillers" | "Antibiotics" | "Antivirals" | "Antifungals" | "Antiparasitics") }))}>
                    {["Painkillers", "Antibiotics", "Antivirals", "Antifungals", "Antiparasitics"].map(option => (
                        <option key={option} value={option}>{option}</option>
                    ))}
                    </select>                
                    <select value={details.medicationUse} onChange={e => setDetails(prev => ({ ...prev, medicationUse: (e.target.value as "Pain Relief" | "Antibacterial" | "Antiviral" | "Antifungal" | "Antiparasitic") }))}>
                    {["Pain Relief", "Antibacterial", "Antiviral", "Antifungal", "Antiparasitic"].map(option => (
                        <option key={option} value={option}>{option}</option>
                    ))}
                    </select>
                    <input type='text' value={details.use} onChange={e => setDetails(prev => ({ ...prev, use: e.target.value }))} />                
                </div>
                ...
                <div className='flex gap-4'>
                    ...
                    <div className='flex flex-col gap-4'>
                        <select value={details.deviceType} onChange={e => setDetails(prev => ({ ...prev, deviceType: (e.target.value as "Wheelchair" | "Crutches" | "Oxygen Tank" | "Blood Pressure Monitor" | "Thermometer") }))}>
                ...
                <>
                    <div className='flex px-12 flex-1 flex-col gap-4'>
                        ...
                        <p className='text-[rgba(0,59,51,1)] text-2xl'>Medication Type: {details.medicationType}</p>
                        <p className='text-[rgba(0,59,51,1)] text-2xl'>Medication Use: {details.medicationUse}</p>
                    </div>
                    <div className='flex flex-1 flex-col items-start gap-4'>
                        <p className='text-[rgba(0,59,51,1)] text-2xl'>Type: {details.type}</p>
                        <p className='text-[rgba(0,59,51,1)] text-2xl'>Use: {details.use}</p>
                ...
                <>
                    <div className='flex px-12 flex-1 flex-col gap-4'>
                        <p className='text-[rgba(0,59,51,1)] text-2xl'>Device Type: {details.deviceType}</p>
                        <p className='text-[rgba(0,59,51,1)] text-2xl'>Gender: {details.gender}</p>
                        <p className='text-[rgba(0,59,51,1)] text-2xl'>Quantity: {details.quantity}</p>
                    </div>
                    <div className='flex flex-1 flex-col items-start gap-4'>
